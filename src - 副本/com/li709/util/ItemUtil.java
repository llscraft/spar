...
public class ItemUtil {
	...
	public static boolean checkSpar(ItemStack gem, ItemStack item,
		...
		for (String str : meta.getLore()) {
			str = str.replaceAll("§.", "").trim();
			if (str.matches(".* \\d+级")) {
				level = Integer.parseInt(str.substring(str.lastIndexOf(" "),
						str.lastIndexOf("级")));
			}
		}
		if (level == null) {
	...
	public static boolean checkHoles(ItemStack item) {
		...
		for (String str : lores) {
			if (str.replaceAll("§.", "").trim().equals(Spar.unperforated)) {
				return true;
			}
		}
		return false;
    ...
    // 如果随机数大于几率
    if (Math.random() > EventListener.ODDS + checkLucky(fuel) / 100) {
      System.out.println("镶嵌失败");
      map.put("物品", item);
    ...
    String lore="●<"+list.get(0)+list.get(1)+"级 +"+list.get(2)+list.get(3)+">";
    ...
    if (index!=null) {
		...
		for (int i = 0; i < lores.size(); i++) {
			if (lores.get(i).replaceAll("§.", "").trim().equals(Spar.unperforated)) {
				lores.set(i, lore);
				break;
			}
		}
	...
	public static List<Object> getGemNameLevelAttribute(List<String> lores) {
		...
		for (String str : lores) {
			str = str.replaceAll("§.", "").trim();
			if (str.matches(".* \\d+级")) {
				list.add(1,Integer.parseInt(str.substring(str.lastIndexOf(" "),
						str.lastIndexOf("级"))));
			}
			if (str.indexOf(Spar.armour_name) != -1) {
				list.add(0,Spar.armour_name);
				num=Spar.armour;
				list.add(3,"护甲");
			} else if (str.indexOf(Spar.critChance_name) != -1) {
				list.add(0,Spar.critChance_name);
				num=Spar.critChance;
				list.add(3,"暴击");
			} else if (str.indexOf(Spar.damage_name) != -1) {
				list.add(0,Spar.damage_name);
				num=Spar.damage;
				list.add(3,"伤害");
			} else if (str.indexOf(Spar.dodge_name) != -1) {
				list.add(0,Spar.dodge_name);
				num=Spar.dodge;
				list.add(3,"闪避");
			} else if (str.indexOf(Spar.health_name) != -1) {
				list.add(0,Spar.health_name);
				num=Spar.health;
				list.add(3,"血量");
			} else if (str.indexOf(Spar.lifesteal_name) != -1) {
				list.add(0,Spar.lifesteal_name);
				num=Spar.lifestealheal;
				list.add(3,"吸血");
			} else if (str.indexOf(Spar.reduction_name) != -1) {
				list.add(0,Spar.reduction_name);
				num=Spar.reduction;
				list.add(3,"减伤");
			}
		}
		list.add(2,num*(Integer)list.get(1));
		return list;
	}
	...
	public static ItemStack checkProtection(ItemStack fuel) {
		...
		for (String string : lores) {
			if (string.equals(Spar.protection_item)) {
				flag = true;
				break;
			}
		}
		if (flag) {
			lores.remove(Spar.protection_item);
			meta.setLore(lores);
